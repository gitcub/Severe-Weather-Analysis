...
```{r, echo=FALSE}
setwd("/Users/rachelstephens/Dropbox/School/Coursera/Reproducible Research/Project 2")
```
...
```{r access and load data, cache = TRUE}
fileURL <- "https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2"
download.file(fileURL, destfile = "dataset.zip", method = "curl")
data <- read.csv("repdata-data-StormData.csv.bz2")
```

There are `r nrow(data)` observations and `r ncol(data)` variables. However, we will subset the data to only look at the variables of interest:
